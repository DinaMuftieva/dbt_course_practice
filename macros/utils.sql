...
--макрос, который будет находить все модели и вьюшки, которые не будут соответствовать ни одной модели из проекта, а потом удалять
{% macro drop_old_relations(dryrun = False) %}
  {# 1. находим все модели, сиды, снэпшоты проекта #}
    {% set current_models = [] %}
  ...
  {# 2. формирование списка объектов, которым не соотвествует ни одна модель, сид, снэпшоты #}
    {% set cleanup_query %}
  ...
  {# 3. удаление лишних/устаревших объектов #}
    {% if drop_commands %}
